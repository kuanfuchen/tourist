...
export default {
  ...
  methods: {
      ...
      this.axios({
        ...
        if (typeSelect === "Restaurant") {
          for (let i = 0; data.length > i; i++) {
            this.restaurantInfo.push(data[i]);
            if (!this.restaurantInfo[i].Picture.PictureUrl1) {
              this.restaurantInfo[i].Picture.PictureUrl1 = this.defaultImg;
            }
          }
        }

        this.pages.total_page = Math.ceil(this.restaurantInfo.length / 20);
        this.pages.page = "1";
        this.getPageInfoFn();
      });
    },
    ...
    getPageInfoFn(page = 1) {
      ...
      if (finalPage > this.restaurantInfo.length) {
        finalPage = this.restaurantInfo.length;
      }
      for (let i = forOriginVal; finalPage > i; i++) {
        this.showCardRestaurantInfo.push(this.restaurantInfo[i]);
      }
      console.log(this.showCardRestaurantInfo);
      this.pageDisplay();
    },
  },
  watch: {
    "$route.params.city": {
      handler(val) {
        this.touristDataFn(val, "Restaurant");
      },
    },
  },
  async mounted() {
    ...
    await this.touristDataFn(cityName, "Restaurant");
